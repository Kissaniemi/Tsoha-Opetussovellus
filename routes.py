...
def create_student():
    ...
    if request.method == "POST":
        ...
        if errors:
            return render_template("register_student.html", message=errors)     
...
def create_teacher():
    ...
    if request.method == "POST":
        ...
        if password1 != password2:
            errors.append("Passwords do not match")
            if errors:
                return render_template("register_teacher.html", message=errors)
...
def create_course():
    if request.method == "GET":
            ...
            if user_type[0] == False:
                return render_template("new_course.html", error="Only teachers can make courses")
    ...
    if request.method == "POST":
        ...
        if session["csrf_token"] != request.form["csrf_token"]:
                return render_template("new_course", message="403")
        if errors:
            return render_template("new_course.html", message=errors)
        user_id = session.get("user_id", 0)
    ...
    if request.method == "POST":
        ...
        if courses.check_course_teacher(teacher_id, int(course_id)):
            if courses.delete_course(course_id):
                return redirect("/courses")
        return render_template("course_page.html", error="Only the course teacher can delete the course")
...
def change_course(course_id):
    if request.method == "GET":
        ...
        if not course_id:
            return render_template(
                "courses.html", message="course id not found")
        teacher_id = session.get("user_id", 0)
    ...
    if request.method == "GET":
        ...
        if not courses.check_course_teacher(user_id, course_id):
            if not courses.check_course_student(user_id, course_id):
                return render_template("student_statistics.html", message="Only students of this course can see the course statistics")
        if user_type[0] == False:
            ...
            for task in course_tasks:
                    answers.append((tasks.get_answers(course_id, task[0])))
...
def add_material(course_id):
    if request.method == "GET":
        ...
        teacher_id = session.get("user_id", 0)
        if not courses.check_course_teacher(teacher_id, course_id):
            return render_template("course_page.html", error="Only the course teacher can add material to this course", course_id=course_id)
        return render_template("new_material.html", course_id=course_id)
    ...
    if request.method == "POST":
        ...
        if errors:
            return render_template("new_material.html", message=errors, course_id=course_id)
        if materials.add_material(course_id, name, material):
            return redirect(url_for("course_page", course_id=course_id))
        else:
            return render_template("course_page.html", message="Creating course material failed")
...
def get_material(course_id):
    user_id = session.get("user_id", 0)
    if user_id == 0:
        return render_template("index.html", message="You need to log in first")
    if not courses.check_course_teacher(user_id, course_id):
        if not courses.check_course_student(user_id, course_id):
            return render_template("courses", message="Only students of this course can see the course material")
...
def material_page(course_id, material_id):
    ...
    if not courses.check_course_teacher(user_id, course_id):
        if not courses.check_course_student(user_id, course_id):
            return render_template("courses.html", message="Only students of this course can see the course material")
...
def delete_material(mat_id):
    if request.method == "POST":
        ...
        return render_template("course_material.html", error="Only the teacher of this course can delete the material")
...
def change_material(material_id):
    if request.method == "GET":
        if request.method == "GET":
            ...
            if user_id == 0:
                return render_template("index.html", error="You need to log in first")
        if not material_id:
            return render_template("materials.html", message="material id not found")
        teacher_id = session.get("user_id", 0)
        info = materials.get_material_info(material_id)
        course_id = info[3]
        if not courses.check_course_teacher(teacher_id, course_id):
            return render_template("materials.html", error="Only the course teacher can change the material")   
    ...
    if request.method == "POST":
        ...
        if materials.change_material(material_id, name, material):
            ...
            course_id = info[3]
            return redirect(f"/course_material/{course_id}")
        return render_template("change_material.html", message="Material change failed")
...
def add_multiple_choice(course_id):
    if request.method == "GET":
        ...
        if user_id == 0:
            return render_template("index.html", error="You need to log in first")
        if not courses.check_course_teacher(user_id, course_id):
            return render_template("courses.html", error="Only the course teacher can add tasks to this course")
    ...
    if request.method == "POST":
        ...
        return render_template("new_multiple_choice_task.html", message="Task creation failed")
...
def add_fill_in(course_id):
    ...
    if request.method == "POST":
        ...
        if tasks.add_task_choice(task_id, answer):
                    return redirect(f"/course_tasks/{course_id}")
        return render_template("new_fill_in_task.html", message="Task creation failed")
...
def choose_task(course_id):
    if request.method == "GET":
        ...
        if not courses.check_course_teacher(user_id, course_id):
            return render_template("courses.html", error="Only the course teacher can add tasks to this course")
        info = courses.get_course_info(course_id)
        return render_template("choose_task.html", info=info, course_id=course_id)
...
def delete_task(task_id):
    if request.method == "POST":
        ...
        return render_template("courses.html", error="Only the teacher of this course can delete tasks")
